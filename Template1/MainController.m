...
- (void)viewDidLoad
{
    ...
    {
        NSMutableArray *_cellArray=[[NSMutableArray alloc]initWithObjects:@" Row 1",@" Row 2",@" Row 3",@" Row 4", nil];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section{
    ...
    if ([[cellArray objectAtIndex:section]count]==0) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    @try {
        ...
        if ([[cellArray objectAtIndex:indexPath.section] count]==0) {
    ...
    @catch (NSException *exception) {
        NSLog(@"Fucking Exception----%@",exception);
    }
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (editingStyle==UITableViewCellEditingStyleDelete)
    {
       ...
       [ [cellArray objectAtIndex:indexPath.section ]removeObjectAtIndex:indexPath.row];

        
        [cellCount replaceObjectAtIndex:indexPath.section withObject:[NSNumber numberWithInt:[[cellArray objectAtIndex:indexPath.section]count]]];
